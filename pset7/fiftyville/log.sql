...
-- license_plates (TEXT): 5P2BI95,94KL13X,6P58WS2,4328GD8,G412CB7,L93JTIZ,322W7JE,0NTHK55
...
-- account_number (INTEGER): 28500762,28296815,76054385,49610011,16153065,25506511,81061156,26013199
...
-- person_id (INTEGER): 686048,514354,458378,395717,396669,467400,449774,438727
...
-- origin_airport_id: 8|CSF|Fiftyville Regional Airport
...
-- Final query used to find the theif
SELECT name FROM people
WHERE id IN (SELECT person_id FROM bank_accounts WHERE account_number IN (SELECT account_number FROM atm_transactions WHERE year= 2020 AND month= 7 AND day= 28
                                                                          AND atm_location= 'Fifer Street' AND transaction_type= 'withdraw'))
AND phone_number IN (SELECT receiver FROM phone_calls WHERE year= 2020 AND month = 7 AND day = 28 AND duration< 60)
